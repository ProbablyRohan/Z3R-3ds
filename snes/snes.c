/*
 * snes.c — SNES system bus and main control logic.
 *
 * Implements the 24-bit address decoder that routes reads and writes to the
 * correct subsystem based on bank and offset:
 *   - $00-$3F/$80-$BF, $0000-$1FFF: WRAM mirror (first 8KB)
 *   - $00-$3F/$80-$BF, $2100-$21FF: B-bus (PPU, APU ports, WRAM access port)
 *   - $00-$3F/$80-$BF, $4016-$4017: Joypad serial ports
 *   - $00-$3F/$80-$BF, $4200-$421F: CPU internal registers (NMI, IRQ, math, joypad)
 *   - $00-$3F/$80-$BF, $4300-$437F: DMA channel registers
 *   - $7E-$7F: Full 128KB WRAM
 *   - Everything else: Cartridge (ROM/SRAM via cart_read/cart_write)
 *
 * Also manages the CPU↔APU synchronization, auto-joypad reading, hardware
 * multiply/divide, and interrupt (NMI/IRQ) timing registers.
 */
...
static void snes_catchupApu(Snes* snes);               // Run APU cycles to sync with CPU
static uint8_t snes_readReg(Snes* snes, uint16_t adr);  // Read CPU-side register ($4210-$421F)
static void snes_writeReg(Snes* snes, uint16_t adr, uint8_t val); // Write CPU-side register ($4200-$420D)
static uint8_t snes_rread(Snes* snes, uint32_t adr);    // Raw read (without open bus update)
static int snes_getAccessTime(Snes* snes, uint32_t adr); // Memory access speed (6/8 cycles)

// Initialize the SNES system: allocate all subsystems and wire them together.
// The `ram` parameter is the caller-provided 128KB WRAM buffer.
...
// Serialize/deserialize the entire SNES state. Visits each subsystem in order,
// then saves the system-level fields (hPos through openBus), WRAM, and ramAdr.
...
// Reset the SNES. Cart resets first (so the CPU can read the reset vector from ROM).
// Hard reset clears all 128KB of WRAM; soft reset preserves it.
...
// Run the APU for all accumulated fractional cycles to synchronize it with
// the CPU. The APU runs at ~1.024 MHz vs the CPU's ~21 MHz, so we batch
// APU cycles and run them when the CPU writes to APU ports or at sync points.
...
// Perform the auto-joypad read sequence that the SNES hardware does at the
// start of vblank. Latches both controllers, then shifts out all 16 bits
// from each port into the portAutoRead[] array (readable at $4218-$421F).
...
// Read from the B-bus ($2100-$21FF mapped as $00-$FF).
// $00-$3F: PPU registers. $40-$7F: APU output ports. $80: WRAM data port.
...
// Write to the B-bus ($2100-$21FF mapped as $00-$FF).
// $00-$3F: PPU registers. $40-$7F: APU input ports (catches up APU first).
// $80-$83: WRAM data port and address registers.
...
// Read a CPU-side internal register ($4210-$421F).
// $4210 (RDNMI): NMI flag + CPU version. Reading clears the NMI flag.
// $4211 (TIMEUP): IRQ flag. Reading clears it and de-asserts the IRQ line.
// $4212 (HVBJOY): Vblank/Hblank/auto-joypad-busy status.
// $4214-$4217: Division result / multiplication result.
// $4218-$421F: Auto-joypad read results for ports 1-4.
...
// Write a CPU-side internal register ($4200-$420D).
// $4200 (NMITIMEN): NMI enable, H/V IRQ enable, auto-joypad enable.
// $4202-$4206: Hardware multiply/divide operands (results computed immediately).
// $4207-$420A: H/V IRQ timer values.
// $420B: Start general DMA. $420C: Enable HDMA channels. $420D: FastROM enable.
...
// Raw 24-bit address space read (does not update open bus — caller does that).
// Decodes the bank:offset into the correct subsystem or falls through to cart.
...
// Write to the 24-bit address space. Routes to WRAM, B-bus, joypad latch,
// CPU registers, DMA registers, or cartridge based on bank and offset.
...
// Public read: raw read + update open bus with the returned value.
...
// CPU-facing read: adds memory access time to the cycle counter, then reads.
...
// CPU-facing write: adds memory access time to the cycle counter, then writes.
